...
import { FALLBACK_IMGS } from '../data';
...
export default function Img({ src, alt = '', className = '', eager = false, style, ...rest }) {
  ...
  const fb = FALLBACK_IMGS[hash(src || '') % FALLBACK_IMGS.length];
  const isFallback = failed && !FALLBACK_IMGS.includes(src);
  return (
    <img
      src={isFallback ? fb : src}
      alt={alt}
      className={className}
